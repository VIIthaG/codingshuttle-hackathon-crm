    ...
    <div className="fixed inset-0 z-40 flex justify-end bg-slate-900/30">
      ...
      <aside className="flex h-full w-full max-w-md flex-col border-l border-border bg-surface shadow-xl">
        ...
        <footer className="flex flex-wrap gap-2 border-t border-border px-5 py-4">
          {active ? (
            <>
              <button type="button" disabled={pending} onClick={() => onComplete(call, outcome.trim() || null)} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm text-white">Complete</button>
          ...
          <button type="button" disabled={pending} onClick={() => onDelete(call)} className="rounded-lg border border-red-200 px-3 py-2 text-sm text-red-600">Delete</button>
        </footer>
      </aside>
    </div>
  )
